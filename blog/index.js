...
//第三个必须条件的验证，大写字母，小写字母，数字，非空字符任意两种混拼即可
	if(/[\d]/.test(value)){
		code_length++;
	}
	if(/[a-z]/.test(value)){
...
$('form').eq(0).form('email').bind('keyup',function(event){
	...
	if(event.keyCode == 40){
		if(this.index == undefined || this.index >= $('#reg .all_email li').length()-1){this.index = 0; 	
		} else {
			this.index++;
		}
	...
	if(event.keyCode == 38){
		if(this.index == undefined || this.index <= 0){this.index = $('#reg .all_email li').length()-1; 	
		} else {
			this.index--;
		}
...
$('form').eq(0).form('sub').click(function(){
	...
	//问题1： 将xsrc地址替换到src中去
	//当图片进入可见区域的时候，将图片的xsrc的地址替换到src即可

	//$('.wait_load').eq(0).attr('src',$('.wait_load').eq(0).attr('xsrc'));


	//问题2： 获取图片元素到最外层定点元素的距离
...
//图片弹窗
	...
	photo_big.center(620,511).resize(
		function (){
...
//IE必须把src这个属性放在load事件的下面才有效
	temp_img.src = 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1490859134771&di=45bc0f612ad668ffc935223 dbe815621&imgtype=0&src=http%3A%2F%2Fss.aibo123.com%2FUploadFiles%2F201409%2F20140912142828941.jpg'; //src属性可以在后台加载这张图片
...
$('#photo_big .big .left').click(function() {
	...
	//$('#photo_big .big img').ge($('#photo_big .big img').attr('index'));
...
$('form').eq(2).form('sub').click(function () {
	...
	ajax({
		...
		success : function (text) {
			...
			$('#index').html(html);
			for(var i = 0; i < json.length; i ++){	$('#index .content').eq(i).animate({
				attr : 'o',
				target : 100,
				t : 30,
				step : 10

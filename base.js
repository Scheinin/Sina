...
//alert(_this);
if(args != undefined){
	//this是一个对象,undefined也是一个对象，区别与typeof返回的带单引号的undefined
...
Base.prototype.animate = function (obj){
	for (var i = 0; i < this.elements.length; i ++){
		...
		if(alter != undefined && target == undefined){
			...
			throw new Error('alter增量或target目标必须传一个！');
		}
		...
		element.timer = setInterval(function(){
			...
			/*
			问题1：多个动画执行了多个列队动画，我们要求不管多少个动画值执行一个列队动画
			问题2：多个动画数值差别他打，导致动画无法执行到目标值，原因是定时器提前清理掉了

			解决1：不管多少个动画，值踢动一次列队动画的机会
			...
			*/
			...
			var flag = true; //表示都执行完毕了
				...
				for (var i in mul) {
					attr = i == 'x' ? 'left' : i == 'w' ? 'width' : i == 'h' ? 'height' : i == 'o' ? 'opacity' : i != undefined ? i : 'left';
					target = mul[i];
